	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`discussqinshixinxi`, this.ruleForm.id);
...
//跨表计算判断
				...
				if(crossrefid && crossuserid) {
					...
					let res = await this.$api.list(`discussqinshixinxi`, params);
                ...
                //跨表计算
						if(this.ruleForm.id){
							await this.$api.update(`discussqinshixinxi`, this.ruleForm);
						}else{
							await this.$api.add(`discussqinshixinxi`, this.ruleForm);
						}
						uni.setStorageSync('discussqinshixinxiCleanType',true);
